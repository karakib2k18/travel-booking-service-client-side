    ...
    const onSubmit = data => {
        handleSignInWithEmailPassword(data.email, data.password)
            .then((result) => {
                ...
                swal("You login successfully!!", data.email, "success", {
                    button: false,
                    timer: 1300
                })
            })
    ...
    const loginFinalWithGmail = () => {
        handleSignInWithGoogle()
            .then((result) => {
                ...
                history.push(redirect_url)
                // console.log('login succesfull')
               swal("You login successfully!!", "done", "success", {
                    button: false,
                    timer: 1300
                })
            })
                    ...
                    <form className="mt-8 space-y-6" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center">
                                <Link to="/signup" className="ml-2 block text-sm text-indigo-600 hover:text-indigo-500">
